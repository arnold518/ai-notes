...
!!! definition "Definition 1 : General Forward Pass"
...
![Forward Pass Diagram](./assets/1.1.png){: .center style="width:100%;"}
[image 1 : forward pass diagram]
...
!!! definition "Definition 2 : Network Error"
...
!!! definition "Definition 3 : General Back Propagation Through Time"
...
!!! concept "Concept 4 : Base BPTT Equations"
...
!!! concept "Concept 5 : $BPTT(h; h')$ Equations"
    ...
    2. Backpropagates only through the last $h$ steps, i.e. it computes **Concept 4** but restricts the recursion to $\tau\in[t-h+1,t]$ and ignores contributions earlier than $t-h$.
...
!!! definition "Definition 6 : Real-Time BPTT"
...
!!! definition "Definition 7 : Epochwise BPTT"
...
!!! definition "Definition 8 : Truncated Real-Time BPTT"
    
    $$BPTT(h, 1)$$

    **Truncated Real-Time BPTT** is the same as real-time bptt (**Definition 6**) but backprop only through the last $h$ steps.

!!! definition "Definition 9 : Truncated Epochwise BPTT"

    $$BPTT(h, \infty)$$

    **Truncated Epochwise BPTT** is the same as epochwise bptt (**Definition 7**) but run an epoch, then backprop only $h$ steps from the end.
    
### Real-Time Recurrent Learning

!!! definition "Definition 10 : Real-Time Recurrent Learning"
    ...
    From chain rule and differentation of **Definition 1** and **Definition 2**, we find that
...
!!! definition "Definition 11 : Teacher Forcing"
...
Thus **Concept 5** changes to

$$
e_k(\tau;J^{\text{total}})=\displaystyle \frac{\partial J(t)}{\partial y_k(t)}, \quad k \in T(\tau)
$$

In RTRL, we treat the value of $p_{i j}^{l}(t)$ as zero for any $l \in T(t)$ when computing $p_{i j}^{k}(t +1)$ via **Definition 10**.
...
!!! concept "Concept 12 : Pros and Cons of Teacher Forcing"
    **Pros**
...
!!! definition "Definition 13 : Pytorch RNN"
...
There is a major difference in the forward pass described in the paper (**Definition 1**, **Definition 2**) with Pytorch's forward pass (**Definition 13**), because of the one timestep delay in **Definition 1**.
Thus, we introduce a universal convention that is based mostly on the paper, but can be interpreted in both situations.

!!! definition "Definition 14 : Universal Forward Pass Convention"
    Paper (**Definition 1**, **Definition 2**)
    ...
    **Universal Convention**
    ...
    - Forward pass :
        `x_list[t] = x(t)`, `s_list[t] = s(t+1)`, `y_list[t] = y(t+1)` ($x(t) = [x^{\text{in}}(t), y(t)], s(t+1)=Wx(t), y(t+1)=f(s(t+1))$)
    - Supervision (targets) : 
        `D[t]=d(t+1)`, At timestep $t$, we compare `y_list[t]` to `D[t]`, i.e. $y(t+1)$ to $d(t+1)$.
    ...
    | Output/hidden            | $y(t+1)$            | `output[t]`$=h_t$ | `y_list[t]`                    |
    | Previous state           | $y(t)$              | `output[t-1]`$=h_{t-1}$ | `x_list[t-1]`      |
    | Target at timestep $t$   | $d(t+1)$            | `answer[t]`       | `D[t]`                         |

    </div>


Using the universal forward pass convention (**Definition 14**), we implemented and compared various algorithms of gradient computation in RNNs.
...
!!! code "Code 2"
    - `alg_pytorch.py` : 
        Used a single layer pytorch rnn module described in **Definition 13**, modified to match **Definition 14**.
    - `alg_bptt.py` : 
        Implemented $BPTT(h, h')$ described in **Definition 3** and **Concept 5**.
        Achieved variants of the bptt algorithm by changing the parameters, $h$ and $h'$.

        - ebptt $\equiv BPTT(\infty, \infty)$ : Epochwise BPTT defined in **Definition 7**.
        - rtbptt $\equiv BPTT(\infty, 1)$ : Real-Time BPTT defined in **Definition 6**.
        - tbptt $\equiv BPTT(h, h')$ : general truncated BPTT.
    - `alg_rtrl.py` : 
        Implemented RTRL described in **Definition 10**.
